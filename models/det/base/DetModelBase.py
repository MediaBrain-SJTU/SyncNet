...
class DetModelBase(nn.Module):
    ...
    def agents_to_batch(self, feats, agent_num=0):
        """Concatenate the features of all agents back into a batch.
    ...
    def feature_transformation(
        b, j, agent_idx, local_com_mat, all_warp, device, size, trans_matrices, LA_Flag = False
    ):
        ...
        if LA_Flag:
            tfm_ji = trans_matrices[b, j]
            warp_feat = nb_agent.squeeze(0)
        else:
