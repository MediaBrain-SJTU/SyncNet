...
class FaFModule(object):
    ...
    def get_trans_matrices(self, current_pose_rec_rotation, current_pose_rec_translation, current_cs_rec_rotation, current_cs_rec_translation, center_agent = 1):
        batch_size, num_agent, k, _ = current_pose_rec_rotation.shape
        trans_to_newest = torch.zeros((batch_size, num_agent, k-1, 4, 4))
        pose_para_list = [[current_pose_rec_rotation[batch], current_pose_rec_translation[batch], current_cs_rec_rotation[batch],current_cs_rec_translation[batch], center_agent] for batch in range(batch_size)]

        
        with futures.ThreadPoolExecutor(None) as executor:
            res = executor.map(self.per_batch_get_transmatrices, pose_para_list, chunksize=batch_size)
            res = list(res)

        # for agent_id in range(num_agent):
        #     for t in range(k):
        #         if t < k-1:
        #             pose_para = [current_pose_rec_rotation[batch][agent_id][t], current_pose_rec_translation[batch][agent_id][t], current_cs_rec_rotation[batch][agent_id][t], current_cs_rec_translation[batch][agent_id][t], ref_from_car, car_from_global]
        #             trans_to_newest[batch][agent_id][t] = self.vector2mat(pose_para)
    ...
    def step(self, data, batch_size, num_agent):
        ...
        if self.LA_Flag == True:
            trans_matrices_newest = data['trans_matrices_newest']
    ...
    def predict_all_with_box_com(self, data, trans_matrices_map, validation=True):
        ...
        for k in range(NUM_AGENT):
            ...
            if torch.nonzero(bev_seq).shape[0] == 0:
                ...
                class_selected, box_scores_pred_cls = apply_nms_det_detectron2(
                    batch_box_preds,
                    batch_cls_preds,
                    anchors,
                    self.code_type,
