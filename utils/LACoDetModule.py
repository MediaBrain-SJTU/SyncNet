...
class FaFModule(object):
    ...
    def step(self, data, batch_size, num_agent, center_agent = 1):
        ...
        bev_supervision = data["bev_supervision"]
        
        if self.LA_Flag == True:
            trans_matrices_supervision = data["trans_matrices_supervision"]
        ...
        if self.config.flag.startswith("when2com") or self.config.flag.startswith(
            ...
            if self.config.split == "train":
                result = self.model(
                    ...
                    bev_supervision,
                    trans_matrices,
                    trans_matrices_newest,
                    trans_matrices_supervision,
                    center_agent,
                    num_all_agents,
            ...
            if self.kd_flag:
                ...
                result = self.model(
                    bev_seq, bev_supervision, trans_matrices, trans_matrices_newest, trans_matrices_supervision, num_all_agents, batch_size=batch_size
                )
        ...
        # loss = loss + kd_loss
        # loss_dict = {}
        # loss_dict['compensation_loss_f'] = result['compensation_loss_f']
        # loss_dict['compensation_loss_f'] = result['compensation_loss_f']
        # loss_dict['cls'] = loss_cls
        # loss_dict['loc'] = loss_loc
        compensation_loss_f = result['compensation_loss_f'] 
        compensation_loss_h = result['compensation_loss_h']
        loss = loss*0.5 + compensation_loss_f*0.1 + compensation_loss_h
        ...
        if self.config.pred_type in ["motion", "center"] and not self.only_det:
            ...
            return loss, loss_cls, loss_loc, compensation_loss_f, compensation_loss_h
    ...
    def predict_all(self, data, batch_size, validation=True, num_agent=5):
        ...
        for k in range(1):
            ...
            bev_seq = torch.unsqueeze(data["bev_seq"][k+1, :, :, :, :], 0)
