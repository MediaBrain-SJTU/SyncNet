...
class CarscenesDataset(Dataset):
    def __init__(self, padded_voxel_points_example, label_one_hot_example, reg_target_example, reg_loss_mask_example,
                 ...
                 config_global=None, agent_list = ['agent0', 'agent1', 'agent2', 'agent3', 'agent4'], split=None, cache_size=10000, val=False, latency_lambda = [10,10,10,10,10], forcast_num = 5):
        """
        This dataloader loads single sequence for a keyframe, and is not designed for computing the
         spatio-temporal consistency losses. It supports train, val and test splits.
    ...
    def __getitem__(self, idx):
        ...
        data_return['center_agent'] = center_agent
        for agent in range(len(self.latency_lambda)):
            data_return[agent] = {}
            seq_file_new = self.seq_dict[center_agent][agent][idx]
            ...
            # seq_file = self.seq_files[idx]
                ...
                if gt_data_handle == 0:
                    ...
                    if self.val:
                        ...
                        data_return[agent]['padded_voxel_points'].append(padded_voxel_points)
                        ...
                        data_return[agent]['filename'] = data_return[agent]['filename'] + seq_file + ','
                        data_return[agent]['target_agent_id'].append(0)
                        data_return[agent]['num_sensor'].append(0)
                        data_return[agent]['trans_matrices'].append(np.zeros((5, 4, 4)))
                        data_return[agent]['padded_voxel_points_global'].append(0)
                        data_return[agent]['reg_target_global'].append(0)
                        data_return[agent]['anchors_map_global'].append(0)
                        data_return[agent]['gt_max_iou_global'].append(0)
                        data_return[agent]['trans_matrices_map'].append(0)
                        continue
                    else:
                ...
                if empty_flag == False:
                    ...
                    if self.val:
                        ...
                        data_return[agent]['anchors_map'].append(anchors_map)
                        data_return[agent]['vis_maps'].append(vis_maps)
                        data_return[agent]['gt_max_iou'].append([ {"gt_box": gt_max_iou}])
                        data_return[agent]['filename'] = data_return[agent]['filename'] + seq_file + ','
                        data_return[agent]['target_agent_id'].append(target_agent_id)
                        data_return[agent]['num_sensor'].append(num_sensor)
                        data_return[agent]['trans_matrices'].append(trans_matrices)
                        data_return[agent]['padded_voxel_points_global'].append(padded_voxel_points_global)
                        data_return[agent]['reg_target_global'].append(reg_target_global)
                        data_return[agent]['anchors_map_global'].append(anchors_map_global)
                        data_return[agent]['gt_max_iou_global'].append([{"gt_box_global": gt_max_iou_global}])
                        data_return[agent]['trans_matrices_map'].append(trans_matrices_map)
                        continue
                    else:
                        ...
                        data_return[agent]['trans_matrices'].append(trans_matrices)
                        if write_flag == True:
                            data_return[agent]['padded_voxel_points_teacher'] = padded_voxel_points_teacher
                            data_return[agent]['label_one_hot'] = label_one_hot
                            data_return[agent]['reg_target'] = reg_target
                            data_return[agent]['reg_loss_mask'] = reg_loss_mask
                            data_return[agent]['anchors_map'] = anchors_map
            ...
            data_return[agent]['padded_voxel_points'] = torch.stack(tuple(torch.Tensor(data_return[agent]['padded_voxel_points'])),0)
            data_return[agent]['filename'] = data_return[agent]['filename'].strip(',')
            data_return[agent]['target_agent_id'] = torch.Tensor(data_return[agent]['target_agent_id'])
            # data_return[agent]['num_sensor'] = torch.Tensor(data_return[agent]['num_sensor'])
            # data_return[agent]['padded_voxel_points_teacher'] = torch.stack(tuple(torch.Tensor(data_return[agent]['padded_voxel_points_teacher'])),0)
            # data_return[agent]['label_one_hot'] = torch.stack(tuple(torch.Tensor(data_return[agent]['label_one_hot'])),0)
            # data_return[agent]['reg_target'] = torch.stack(tuple(torch.Tensor(data_return[agent]['reg_target'])),0)
            # data_return[agent]['reg_loss_mask'] = torch.stack(tuple(torch.Tensor(data_return[agent]['reg_loss_mask'])),0)
            # data_return[agent]['anchors_map'] = torch.stack(tuple(torch.Tensor(data_return[agent]['anchors_map'])),0)
            # data_return[agent]['vis_maps'] = torch.stack(tuple(torch.Tensor(data_return[agent]['vis_maps'])),0)
            data_return[agent]['trans_matrices'] = torch.stack(tuple(torch.Tensor(data_return[agent]['trans_matrices'])),0)

            # data_return[key].pop('padded_voxel_points')
            # data_return[agent].pop('padded_voxel_points_teacher')
            # data_return[agent].pop('label_one_hot')
            # data_return[agent].pop('reg_target')
            # data_return[agent].pop('reg_loss_mask')
            # data_return[agent].pop('anchors_map')
            # data_return[agent].pop('vis_maps')
            # data_return[agent].pop('filename')
            # data_return[agent].pop('target_agent_id')
            # data_return[agent].pop('num_sensor')
            # data_return[agent].pop('trans_matrices')
            # data_return[agent].pop('gt_max_iou')
            print("item读取运行时间:",time.time() - time_start )
        return data_return
