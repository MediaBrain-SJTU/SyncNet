...
class CarscenesDataset(Dataset):
    ...
    def get_data_dict(self, file_path):
        ...
        for file_name in os.listdir(file_path):
            ...
            if int(time_stamp) >= 10 and int(time_stamp) < 100:
        ...
        if seq_file not in self.cache.keys():
            ...
            if gt_data_handle == 0:
                ...
                if self.val:
                    ...
                    data_return['trans_matrices'] = np.zeros((self.num_agent, 4, 4))
                    ...
                    # cache['num_sensor'] = 0
                    # cache['trans_matrices'] = np.zeros((self.num_agent, 4, 4))
    ...
    def process_supervise_data(self, raw_data):
        ...
        # for j in range(len(tuple(process_data['trans_matrices']))):
        #     print(tuple(process_data['trans_matrices'][j].shape))
    ...
    def __getitem__(self, idx):
        ...
        supervise_load_list = []
        # newest_id = 
        scene_time_index = self.seq_dict[self.center_agent][self.center_agent][idx].split('/')[-1]
