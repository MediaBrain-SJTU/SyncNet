        ...
        while channel_flag == True:
            ...
            if config.split == 'val' or config.split == 'test':
               ...
               if flag_init == False:
                  ...
                  LidarPointCloud.from_file_multisweep_globalmap_sample_data(0, 0, nusc, curr_sample_data, flag_init=flag_init, return_trans_matrix=True)
                  flag_init = True
               else:
                  all_pc_global, trans_matrices_map, _, _, x_filter, y_filter, num_sensor = \
                  LidarPointCloud.from_file_multisweep_globalmap_sample_data(x_origin, y_origin, nusc, curr_sample_data, flag_init=flag_init, return_trans_matrix=True)
               ...
               if current_agent == 0:
                  ...
                  for ann_token in corresponding_sample_rec['anns']:
                      ...
                      instance_boxes_global, instance_all_times_global, _, _ = LidarPointCloud. \
                          get_instance_boxes_upperbound_sample_data(x_origin, y_origin, x_filter, y_filter, nusc, curr_sample_data,
                                                                 instance_token,
                                                                 nsweeps_back=config.nsweeps_back,
                                                                 nsweeps_forward=config.nsweeps_forward)
                  ...
                  save_box_dict_list_global.append(box_data_dict_global)  # we only have one copy of ground-truth bounding box
...
def convert_to_dense_bev(seq_data_dict,config):
    ...
    for i in range(num_sweeps):
        res, voxel_indices = voxelize_occupy(pc_list[i], voxel_size=config.voxel_size, extents=config.area_extents, return_indices=True)
    ...
    for i in range(num_sweeps):
        res_teacher, voxel_indices_teacher = voxelize_occupy(pc_list[i], voxel_size=config.voxel_size, extents=config.area_extents, return_indices=True)
    ...
    Generate_object_detection_gt(gt_dict,config.voxel_size, config.area_extents,config.anchor_size,config.map_dims,\
        config.pred_len,config.nsweeps_back,config.box_code_size,config.category_threshold,config)
    ...
    if config.split == 'val' or config.split == 'test':
       ...
       for i in range(num_sweeps):
           res_global, voxel_indices_global  = voxelize_occupy(pc_list[i], voxel_size=config_global.voxel_size, extents=config_global.area_extents, return_indices=True)
       ...
       if target_agent_id == 0:
          _, reg_target_global, allocation_map_global, gt_max_iou_global, reg_loss_mask_global, _ = \
              Generate_object_detection_gt(gt_dict_global, config_global.voxel_size, config_global.area_extents, config_global.anchor_size, config_global.map_dims,\
                  config_global.pred_len, config_global.nsweeps_back, config_global.box_code_size, config_global.category_threshold, config_global)
       else:
...
def convert_to_sparse_bev(config,dense_bev_data,use_motion_state = False):
    ...
    recover[vis_occupy_indices[0,:],vis_occupy_indices[1,:],vis_occupy_indices[2,:],vis_occupy_indices[3,:]] = math.log(0.7/(1-0.7))
    recover[vis_free_indices[0,:],vis_free_indices[1,:],vis_free_indices[2,:],vis_free_indices[3,:]] = math.log(0.4/(1-0.4))
