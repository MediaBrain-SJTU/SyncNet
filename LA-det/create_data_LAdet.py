...
def create_data(para):
    
    config, nusc, current_agent, config_global, scene_list = para
    channel = "LIDAR_TOP_id_" + str(current_agent)
    total_sample = 0
    
    for scene_idx_float in scene_list:
        scene_idx = int(scene_idx_float)
        ...
        while channel_flag:
            ...
            (
                ...
                current_agent, nusc, curr_sample_data, return_trans_matrix=True, LA_flag=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r",
        "--root",
        default="/GPFS/public/V2X-Sim-2.0",
        type=str,
    ...
    parser.add_argument("-e", "--scene_end", default=2, type=int, help="scene_end")
    parser.add_argument("-t", "--scene_txt", default='/GPFS/data/zxlei/CollaborativePerception/Forcast/LatencyVersion/LA-Coperception/V2XSim/coperception/utils/test_scenes.txt', type=str, help="scene_txt")
    parser.add_argument(
        "-p",
        "--savepath",
        default="/GPFS/public/V2XSim2LA",
        type=str,
    ...
    scene_list = np.loadtxt(args.scene_txt)
    create_list = []
    for current_agent in range(args.from_agent, args.to_agent):
        ...
        create_list.append([config, nusc, current_agent, config_global, scene_list])
        create_data([config, nusc, current_agent, config_global, scene_list])
    # with futures.ThreadPoolExecutor(None) as executor:
    #     res = executor.map(create_data, create_list, chunksize=64)
    #     res = list(res)